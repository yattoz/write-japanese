...
import dmeeuwis.kanjimaster.logic.drawing.PathCalculator;
...
public class DrawView extends View implements OnTouchListener {
    ...
    private List<List<dmeeuwis.kanjimaster.logic.data.Point>> convertPoints(List<List<Point>> drawing){
        List<List<dmeeuwis.kanjimaster.logic.data.Point>> newDrawing = new ArrayList<>(drawing.size());
        for(List<Point> l2: drawing){
            List<dmeeuwis.kanjimaster.logic.data.Point> newStroke = new ArrayList<>(l2.size());
            for(Point p: l2){
                newStroke.add(new dmeeuwis.kanjimaster.logic.data.Point(p.x, p.y));
            }
